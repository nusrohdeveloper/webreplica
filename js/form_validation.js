...
function checkAvailability() {
  ...
  var format_username ="<small>*Username: Letter or combination of letters and numbers and not less than 5 characters and not more than 15 characters</small>"

  if(!isNaN(username_value) || username_value.length < 5 || username_value.length > 15)
  {
    indicator = "<i class='fa fa-times' style='color:red'></i>";
...
function checkEmailAvailability() {
  ...
  var format_email ="<small>*E-Mail: someone@example.com</small>"

  if(!isNaN(email_value) || !email_value.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/))
  {
    indicator = "<i class='fa fa-times' id='email-not-available' style='color:red'></i>";
    $("#email-availability-status").html(indicator);
  }
  if(email_value != '' && isNaN(email_value) && email_value.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/))
  {
    $("#question-icon").hide();
    jQuery.ajax({
    ...
    email.parent().parent().addClass('has-error');
    email.focus();
    $("#email-if-error").html(format_email);
    $("#email-if-error").show();
